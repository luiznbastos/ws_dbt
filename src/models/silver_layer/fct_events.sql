WITH event_main as (
    select * from {{ref('stg_event_main')}}
...
event_qualifiers AS (
    SELECT
        ...
        id,
        qualifier_type
    FROM {{ref('stg_event_qualifiers')}}
    where match_id in (select match_id from event_main)
),
...
SELECT
    ...
    CASE WHEN type ILIKE 'save' AND (
            ...
            OR JSON_SERIALIZE(qualifier_type) LIKE '%ParriedDanger%'
    ...
    CASE WHEN type ILIKE '%dispossessed%'
        ...
        OR (is_touch = True 
            AND outcome_type ILIKE 'unsuccessful' 
            AND JSON_SERIALIZE(satisfied_types) NOT LIKE '%"turnover"%'
            AND type NOT ILIKE 'pass' 
            AND type NOT ILIKE '%TakeOn%' 
            AND type NOT ILIKE '%shot%' 
            AND type NOT ILIKE 'goal'
            AND type NOT ILIKE '%aerial%'
            AND type NOT ILIKE '%foul%'
            AND type NOT ILIKE '%tackle%'
            AND type NOT ILIKE '%interception%')
